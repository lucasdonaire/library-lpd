...
def area_aproximada(f, a, b, eps=EPSILON):
    ...
    # remova ou modifique a linha abaixo como desejar
    k = 2
    dif = erro_rel(area_por_retangulos(f, a, b, 1), area_por_retangulos(f, a, b, 2))
    while dif >= eps:
        dif = erro_rel(area_por_retangulos(f, a, b, k) , area_por_retangulos(f, a, b, 2*k))
        k *= 2
    
    return k, area_por_retangulos(f, a, b, k) 
