...
def conte_vizinhos(M, lin, col):
    ...
    for i in range(lin-1, lin+2):
        for j in range(col-1, col+2):
            if  (0 <= i < taml) and (0 <= j < tamc) and ((i,j)!=(lin,col)):
                if M[i][j] == c:
                    cont += 1
...
def ache_infelizes(M, minviz):
    ...
    for i in range(nlin):
        for j in range(ncol):
            if conte_vizinhos(M,i,j) < minviz and M[i][j] == AZUL:
                lazul += [[i,j]]
            if conte_vizinhos(M,i,j) < minviz and M[i][j] == VERMELHO:
                lverm += [[i,j]]
...
def atualize_mapa(M, origem):
    ...
    qtd = len(origem)
    for i in range(qtd):
        vi, vj = sorteie_casa_vazia(M)
        M[vi][vj] , M[origem[i][0]][origem[i][1]] = M[origem[i][0]][origem[i][1]],  M[vi][vj]
        mud += [[vi, vj]]
